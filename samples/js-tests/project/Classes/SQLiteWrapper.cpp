...
/**
 * init sqlite 
 * distinguish android ios file path , through the path, open the file
 */
std::string SQLiteWrapper::copyToWritablePath(const std::string &db_file){
	...
	unsigned long found = copyPath.find_last_of("/\\");
	std::string dbFilePath = "";
	if (found != std::string::npos)
	{
		dbFilePath = copyPath.substr(0, found+1);
		FileUtils::getInstance()->createDirectories(dbFilePath);
	}
